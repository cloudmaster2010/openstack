...
# Functional Test
#### Create containers with zun command, and test the network connection between provider network.    
...
3. Check the list of containers with zun and docker command.
```sh
$ zun list
+--------------------------------------+----------+--------+---------+------------+-----------------+-------+
...
```sh
stack@zun-com1:~/devstack$ docker ps | grep zun
...
```sh
stack@zun-com1:~/devstack$ docker exec -it zun-4a3738ed-740f-4c47-9dc6-3db986054906 df -h
...
#### Check how docker networks are connected with neutron networks with CLI. 
...
```sh
stack@zun-com1:~/devstack$ docker network ls
...
```sh
stack@zun-com1:~/devstack$ docker network inspect aab6f9bb-fbc8-4eb9-837c-d3e5179c756a
[
    {
